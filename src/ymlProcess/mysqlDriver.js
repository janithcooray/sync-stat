 ...
 */
 ...
 export default class MysqlDriver extends YamlVersion {
	 ...
	 importFromDocker(file){
		 let container = this.yml.database.local.container_name;
		 child_process.execSync('docker cp ' +file+ ' ' + container + ':/datadump.sql' );
		 child_process.execSync('docker exec ' + container + ' mysql -u -p ');
	 }
